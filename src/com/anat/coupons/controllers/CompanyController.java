...
//--------------------------------------------------------------------validations-------------------------------------------------------------------------
	...
	private void validateGetAllCompanies() throws ApplicationException {
		...
		if (this.companyDao.getAllCompanies() == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in companayController: failed to get all companies- The list of companies is empty " + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateLogin(String companyName, String companyPassword) throws ApplicationException {
		...
		if (this.companyDao.checkLogin(companyName, companyPassword) == null) {
			throw new ApplicationException(ErrorType.INVALID_LOGIN, "error in companayController: invalid login of company. username & password do not match." +DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateCompanyId(long companyId) throws ApplicationException {
		...
		if (!this.companyDao.doesCompanyExistById(companyId)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER, "error in companayController: companyId doesnt exist." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateCompanyName(String name) throws ApplicationException {
		...
		if (this.companyDao.doesCompanyNameExist(name)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in companayController: validation of updateName failed: the comapny name you entered already exists. "
							+ DateUtils.getCurrentDateAndTime());
		}
		// we check if the new name is valid
		if (name.length() < 2 || name.length() > 50) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in companayController: validation of updatName failed: company's name is either too short or too long. companys name must be between 2 and 50 characters.");
		}
	}
	...
	private void validateCompanyDetails(Company company) throws ApplicationException {
		...
		if (!LoginValidationUtils.emailValidation(company.getCompanyEmail())) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in companayController: creation/update of company failed: email address is not valid.");
		}
		// we check if the password is valid (via RegExp)
		if (!LoginValidationUtils.passwordValidation(company.getCompanyPassword())) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in companayController: creation/update of company failed: password is not valid. password must contain 8-10 characters, and at least 1 upperCase, 1 lowerCase, and 1 digit. it cannot contain special chars");
		}
