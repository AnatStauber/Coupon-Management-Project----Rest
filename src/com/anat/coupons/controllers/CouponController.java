...
//			CouponType type = coupon.getType();
		...
		if (!this.companyDao.doesCompanyExistById(companyId)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: companyId doesnt exist. " + DateUtils.getCurrentDateAndTime());
		}
	}
...
//			String couponImage = coupon.getCouponImage();
		...
		if (!oldStartDate.equals(couponStartDate)) {
			throw new ApplicationException(ErrorType.ACTION_CANNOT_BE_COMPLETED,
					"error in couponController: forbidden action, you are not allowed to change the coupon start Date. "
		...
		if ((DateUtils.isDate1AfterDate2(couponEndDate, oldEndDate)) && (couponAmount == 0)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: forbidden action. you are not allowed to postpone the coupon's expiry date when there are no coupons left in stock.");
		}
	...
	private void validateGetCouponsByType(CouponType type) throws ApplicationException {
		...
		if (couponDao.getCouponsByType(type) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController - getting coupons by type failed:  seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	...
	private void validateGetCouponByEndDate(String endDate) throws ApplicationException {
		...
		if (!(DateUtils.isDateInFormat(endDate))) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: getting coupons by end date failed. the given endDate is not in the right format: 'yyyy-mm-dd'"
							+ DateUtils.getCurrentDateAndTime());
		}

		if (couponDao.getCouponsByEndDate(endDate) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController - getting coupons by end date failed: seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateGetCouponsByPrices(double minPrice, double maxPrice) throws ApplicationException {
		...
		if (minPrice >= maxPrice) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: invalid parameter- maximum price must be larger than minimum price."
							+ DateUtils.getCurrentDateAndTime());
		}
		// checking that the prices are in the right range
		if ((minPrice < 0) || (maxPrice < 0)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: prices cannot be negative." + DateUtils.getCurrentDateAndTime());
		}

		// checking if the list is not empty
		if (couponDao.getCouponsByPrices(minPrice, maxPrice) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController- failed to get coupons by prices: seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateGetAllActiveCoupons() throws ApplicationException {
		...
		if (couponDao.getAllActiveCoupons() == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController- failed to get all active coupons: seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	...
	private void validateGetAllCoupons() throws ApplicationException {
		...
		if (this.couponDao.getAllCoupons() == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController- failed to get all coupons: seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateGetActiveCouponsByCompany(long companyId) throws ApplicationException {
		...
		if (couponDao.getActiveCouponsByCompany(companyId) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController- failed to get all active coupons of company : seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateGetAllCouponsByCompany(long companyId) throws ApplicationException {
		...
		if (couponDao.getAllCouponsByCompany(companyId) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController- failed to get all coupons of company : seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	...
	private void validateGetActiveCouponsByCustomer(long customerId) throws ApplicationException {
		...
		if (!this.customerDao.doesCustomerExistById(customerId)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: customerId doesnt exist." + DateUtils.getCurrentDateAndTime());
		}
		if (couponDao.getActiveCouponsByCustomer(customerId) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController- failed to get all active coupons of customer : seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateGetAllCouponsByCustomer(long customerId) throws ApplicationException {
		...
		if (couponDao.getAllCouponsByCustomer(customerId) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in couponController- failed to get all coupons of customer : seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validatePurchaseCoupon(long customerId, long couponId, int purchasedAmount)
		...
		if (purchasedAmount <= 0) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController - purchase method: illegal amount inserted. amount must be above 0." + DateUtils.getCurrentDateAndTime());
		}
		...
		if (coupon.getCouponAmount() < purchasedAmount) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER, "error in couponController - purchase method: there are not enough coupons.there are "
	...
	private void validateCouponId(long couponId) throws ApplicationException {
		...
		if (!this.couponDao.doesCouponExistById(couponId)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: couponId doesnt exist. " + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateCouponTitle(String title, long companyId) throws ApplicationException {
		...
		if (title.length() < 2 || title.length() > 45) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: coupon's name is either too short or too long. coupon title must be between 2 and 45 characters."
							+ DateUtils.getCurrentDateAndTime());
		}
		...
		if (this.couponDao.doesCouponTitleExist(title, companyId)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: coupon's title already exists for this company." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateCouponDates(String startDate, String endDate) throws ApplicationException {
		...
		if ((!DateUtils.isDateInFormat(endDate) || (!DateUtils.isDateInFormat(startDate)))) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: validating the date failed. the given Date/s is not in the right format: 'yyyy-mm-dd'"
							+ DateUtils.getCurrentDateAndTime());
		}
...
//				}
		...
		if (DateUtils.isCurrentDateAfterEndDate(endDate)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: the coupon end date has already passed. enter a new end date "
							+ DateUtils.getCurrentDateAndTime());
		}
		// validating that the end date is later than the start date
		if (DateUtils.isDate1AfterDate2(startDate, endDate)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: start date is later than end date " + DateUtils.getCurrentDateAndTime());
		}
	...
	private void validateCouponAmount(int amount) throws ApplicationException {
		...
		if (amount < 0) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: illegal amount entered.amount must be 0 or above." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateCouponMessage(String message) throws ApplicationException {
		...
		if (message.length() < 2 || message.length() > 200) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: coupon's message is either too short or too long. coupon message must be between 2 and 200 characters."
							+ DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateCouponPrice(double price) throws ApplicationException {
		...
		if (price <= 0) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in couponController: the price is not valid. must be higher than 0." + DateUtils.getCurrentDateAndTime());
		}
	}
