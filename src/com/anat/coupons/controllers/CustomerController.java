...
//--------------------------------------------------------------------validations-------------------------------------------------------------------------
	...
	private void validateGetAllCustomers() throws ApplicationException {
		...
		if (this.customerDao.getAllCustomers() == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"error in customerController: failed to get all customers. The list of customers is empty " + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validateLogin(String customerName, String customerPassword) throws ApplicationException {
		...
		if (customerId == null) {
		throw new ApplicationException(ErrorType.INVALID_LOGIN, "error in customerController: unsuccessful login. customer's userName or password are incorrect. ");
		}
	}
	...
	private void validateCustomerId(long customerId) throws ApplicationException {
		...
		if (!this.customerDao.doesCustomerExistById(customerId)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER, "cerror in customerController: ustomerId doesnt exist.");
		}
	}
	...
	private void validateCustomerName(String name) throws ApplicationException {
		...
		if (this.customerDao.doesCustomerNameExist(name)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in customerController: validation of name failed. the customer's name you entered already exists. "
							+ DateUtils.getCurrentDateAndTime());
		}
		// we check if the name is valid
		if (name.length() < 2 || name.length() > 50) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"error in customerController: validation of name failed. customer's name is either too short or too long. customers name must be between 2 and 50 characters.");
		}
	}
	...
	private void validateCustomerPassword (String password) throws ApplicationException {
		...
		if (!LoginValidationUtils.passwordValidation(password)){
				throw new ApplicationException(ErrorType.INVALID_PARAMETER,"error in customerController: creation of customer failed: password is not valid. password must contain 8-10 characters, and at least 1 upperCase, 1 lowerCase, and 1 digit. it cannot contain special chars");
			}
	}
